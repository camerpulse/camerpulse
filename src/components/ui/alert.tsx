  ...
  "relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground transition-all duration-200",
  {
    variants: {
      variant: {
        ...
        warning:
          "border-secondary/50 text-secondary-foreground bg-secondary/5 [&>svg]:text-secondary-foreground",
        civic:
          "border-primary bg-gradient-civic/10 text-primary [&>svg]:text-primary shadow-elegant"
      },
    },
    defaultVariants: {
      variant: "default",
