-- Fix suspect sampling (no LIMIT in aggregate) and compute null ratio separately
...
BEGIN
  ...
  LOOP
    ...
    IF v_match_count IS NULL OR v_match_count = 0 THEN
      -- Compute null ratio for first text column and log suspects if very sparse
      IF v_text_cols IS NOT NULL THEN
        v_sql := format('SELECT COALESCE(AVG(CASE WHEN %1$I IS NULL OR %1$I = '''' THEN 1.0 ELSE 0.0 END),0.0)
                         FROM (SELECT %1$I FROM public.%2$I LIMIT 200) s', v_text_cols[1], r_table.table_name);
        EXECUTE v_sql INTO v_null_ratio;
        IF v_null_ratio > 0.8 THEN
          v_sql := format('SELECT array_agg(%1$I) FROM (SELECT %1$I FROM public.%2$I WHERE %1$I IS NOT NULL ORDER BY %1$I LIMIT 5) s', v_pk_name, r_table.table_name);
          EXECUTE v_sql INTO v_suspect_ids;
          INSERT INTO public.cleanup_backup__suspects (run_id, table_name, pk_sample, reason, details)
          VALUES (v_run_id, r_table.table_name, v_suspect_ids, 'high_null_ratio', jsonb_build_object('threshold', 0.8, 'column', v_text_cols[1]));
        END IF;
      END IF;
      CONTINUE;
    END IF;
