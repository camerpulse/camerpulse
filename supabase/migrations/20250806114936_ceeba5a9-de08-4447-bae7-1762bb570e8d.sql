-- Update existing records with generated slugs (simplified for existing columns)
...
-- For villages table (use whatever column exists)
UPDATE public.villages 
SET slug = generate_slug(COALESCE(village_name, '') || '-' || region, id::text) 
WHERE slug IS NULL AND village_name IS NOT NULL;
