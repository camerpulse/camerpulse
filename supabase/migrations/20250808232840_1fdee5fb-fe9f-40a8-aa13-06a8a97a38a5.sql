-- 1) Suspects table for audit-only flags
CREATE TABLE IF NOT EXISTS public.cleanup_backup__suspects (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  run_id uuid REFERENCES public.cleanup_backup__cleanup_runs(id) ON DELETE CASCADE,
...
-- 2) Enhanced cleanup function: more patterns, soft-delete, suspects logging
...
DECLARE
  ...
  v_has_is_deleted boolean := false;
  r_ref record;
  v_null_ratio numeric := 0.0;
  v_suspect_ids uuid[];
BEGIN
  ...
  LOOP
    ...
    IF v_pk_count <> 1 THEN CONTINUE; END IF;
    ...
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=r_table.table_name AND column_name='verification_status') INTO v_has_verification_status;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=r_table.table_name AND column_name='verified') INTO v_has_verified;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=r_table.table_name AND column_name='is_verified') INTO v_has_is_verified;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=r_table.table_name AND column_name='is_verified_vendor') INTO v_has_is_verified_vendor;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=r_table.table_name AND column_name='deleted_at') INTO v_has_deleted_at;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=r_table.table_name AND column_name='is_deleted') INTO v_has_is_deleted;
    ...
    -- Exclusion for verified content
    v_excl := 'TRUE';
    IF p_exclude_verified THEN
      IF v_has_verification_status THEN v_excl := v_excl || ' AND COALESCE((verification_status)::text, '''') <> ''verified'''; END IF;
      IF v_has_verified THEN v_excl := v_excl || ' AND COALESCE(verified, false) = false'; END IF;
      IF v_has_is_verified THEN v_excl := v_excl || ' AND COALESCE(is_verified, false) = false'; END IF;
      IF v_has_is_verified_vendor THEN v_excl := v_excl || ' AND COALESCE(is_verified_vendor, false) = false'; END IF;
    END IF;

    -- Avoid FK violations (no children referencing)
    v_not_refd := '';
    FOR r_ref IN 
      ...
      WHERE c.confrelid = format('public.%I', r_table.table_name)::regclass
        AND c.contype = 'f'
    LOOP
      IF v_not_refd = '' THEN v_not_refd := ' AND TRUE'; END IF;
    ...
    IF v_match_count IS NULL OR v_match_count = 0 THEN
      -- Also log suspects with high NULL ratios (incomplete records)
      IF v_text_cols IS NOT NULL THEN
        v_sql := format('SELECT array_agg(%1$I ORDER BY %1$I LIMIT 5)
                          FROM public.%2$I
                          WHERE (SELECT (COUNT(*) FILTER (WHERE %3$s IS NULL)::numeric / NULLIF(COUNT(*),0))
                                 FROM (SELECT %3$s FROM public.%2$I WHERE %3$s IS NOT NULL LIMIT 50) t) > 0.8',
                        v_pk_name, r_table.table_name, v_text_cols[1]);
        EXECUTE v_sql INTO v_suspect_ids;
        IF v_suspect_ids IS NOT NULL THEN
          INSERT INTO public.cleanup_backup__suspects (run_id, table_name, pk_sample, reason, details)
          VALUES (v_run_id, r_table.table_name, v_suspect_ids, 'high_null_ratio', jsonb_build_object('threshold', 0.8));
        END IF;
      END IF;
      CONTINUE;
    END IF;
    ...
    IF v_text_cols IS NOT NULL AND array_length(v_text_cols,1) > 0 THEN
      v_sql := format(
        'SELECT array_agg(%1$I), jsonb_agg(jsonb_build_object(''sample_col'', %2$I)) FROM (
         ...
         ) s', v_pk_name, v_text_cols[1], r_table.table_name, v_where);
    ELSE
      v_sql := format(
        'SELECT array_agg(%1$I), jsonb_build_array() FROM (
           SELECT %1$I FROM public.%2$I WHERE %3$s LIMIT 5
         ) s', v_pk_name, r_table.table_name, v_where);
    END IF;
    ...
    IF v_has_deleted_at OR v_has_is_deleted THEN
      IF v_has_deleted_at THEN
        v_sql := format('UPDATE public.%I SET deleted_at = now() WHERE %s', r_table.table_name, v_where);
      ELSE
        v_sql := format('UPDATE public.%I SET is_deleted = true WHERE %s', r_table.table_name, v_where);
      END IF;
      EXECUTE v_sql USING p_patterns;
