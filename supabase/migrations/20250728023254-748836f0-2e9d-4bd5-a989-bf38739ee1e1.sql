-- Phase 6E: Integration & API Features (Complete)
...
-- Third-party integration logs (create after dependencies)
CREATE TABLE public.integration_logs_v2 (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_integration_logs_v2_integration ON public.integration_logs_v2(integration_id);
CREATE INDEX idx_integration_logs_v2_timestamp ON public.integration_logs_v2(created_at);
...
ALTER TABLE public.integration_logs_v2 ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can view their own integration logs v2"
  ON public.integration_logs_v2 FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM user_integrations ui 
      WHERE ui.id = integration_logs_v2.integration_id 
      AND ui.user_id = auth.uid()
    ) OR
    EXISTS (
      SELECT 1 FROM webhook_endpoints we 
      WHERE we.id = integration_logs_v2.webhook_endpoint_id 
      AND we.user_id = auth.uid()
    ) OR
    EXISTS (
      SELECT 1 FROM api_keys ak 
      WHERE ak.id = integration_logs_v2.api_key_id 
      AND ak.user_id = auth.uid()
    )
  );

CREATE POLICY "System can insert integration logs v2"
  ON public.integration_logs_v2 FOR INSERT
  WITH CHECK (true);
