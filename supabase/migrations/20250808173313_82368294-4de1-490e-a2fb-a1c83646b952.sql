-- Cleanup migration: archive-and-delete mock/test/demo data across public schema

-- 1) Create backup schema and logging tables
CREATE SCHEMA IF NOT EXISTS cleanup_backup;

CREATE TABLE IF NOT EXISTS cleanup_backup.cleanup_runs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  run_started_at timestamptz NOT NULL DEFAULT now(),
  run_ended_at timestamptz,
  patterns text[] NOT NULL,
  excluded_verified boolean NOT NULL DEFAULT true,
  tables_processed int NOT NULL DEFAULT 0,
  total_deleted int NOT NULL DEFAULT 0,
  notes text
);

CREATE TABLE IF NOT EXISTS cleanup_backup.cleanup_reports (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  run_id uuid NOT NULL REFERENCES cleanup_backup.cleanup_runs(id) ON DELETE CASCADE,
...
-- 2) Create dynamic cleanup function
...
BEGIN
  -- Start run
  INSERT INTO cleanup_backup.cleanup_runs (patterns, excluded_verified)
  ...
  FOR r_table IN 
    ...
    WHERE table_schema = 'public' AND table_type = 'BASE TABLE'
      ...
      AND table_name NOT LIKE 'test_%' -- skip any temp test tables if present
  LOOP
    ...
    -- Count matches
    v_sql := format('SELECT COUNT(*) FROM %I.%I WHERE (%s) AND (%s)', 'public', r_table.table_name, v_cond, v_excl);
    ...
    v_backup_table := format('cleanup_backup.%s', r_table.table_name);
    v_sql := format('CREATE TABLE IF NOT EXISTS cleanup_backup.%I (LIKE public.%I INCLUDING DEFAULTS)', r_table.table_name, r_table.table_name);
    EXECUTE v_sql;

    -- Backup rows
    v_sql := format('INSERT INTO cleanup_backup.%I SELECT * FROM public.%I WHERE (%s) AND (%s)', r_table.table_name, r_table.table_name, v_cond, v_excl);
    EXECUTE v_sql USING p_patterns;

    -- Collect sample ids and text if available (first text col)
    IF v_text_cols IS NOT NULL AND array_length(v_text_cols,1) > 0 THEN
      v_sql := format('SELECT array_agg(%1$I LIMIT 5), jsonb_agg(jsonb_build_object(''sample_col'', %2$I) LIMIT 5) FROM public.%3$I WHERE (%4$s) AND (%5$s)', 
                      v_pk_name, v_text_cols[1], r_table.table_name, v_cond, v_excl);
    ELSE
      v_sql := format('SELECT array_agg(%1$I LIMIT 5), jsonb_build_array() FROM public.%2$I WHERE (%3$s) AND (%4$s)', 
                      v_pk_name, r_table.table_name, v_cond, v_excl);
    END IF;
    EXECUTE v_sql USING p_patterns INTO v_sample_ids, v_sample_text;

    -- Delete rows
    v_sql := format('DELETE FROM public.%I WHERE (%s) AND (%s)', r_table.table_name, v_cond, v_excl);
    EXECUTE v_sql USING p_patterns;
    ...
    -- Report per table
    INSERT INTO cleanup_backup.cleanup_reports (
  ...
  -- Finalize run
  UPDATE cleanup_backup.cleanup_runs
...
-- 3) Execute cleanup once now and store result in reports table context (returned as a notice)
DO $$
DECLARE
  v_result jsonb;
BEGIN
