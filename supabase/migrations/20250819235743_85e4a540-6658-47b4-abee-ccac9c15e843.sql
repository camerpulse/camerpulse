...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'cleanup_scan_reports' AND polname = 'Admins can manage cleanup_scan_reports'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'cleanup_scan_runs' AND polname = 'Admins can manage cleanup_scan_runs'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'user_migration_log' AND polname = 'Admins can manage user_migration_log'
  ) THEN
...
BEGIN
  ...
  LOOP
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE schemaname = 'public' AND tablename = r.table_name AND polname = format('Admins can manage %s', r.table_name)
    ) THEN
      EXECUTE format('CREATE POLICY "%s" ON public.%I FOR ALL USING (public.has_role(auth.uid(), ''admin'')) WITH CHECK (public.has_role(auth.uid(), ''admin''));',
        format('Admins can manage %s', r.table_name), r.table_name);
    END IF;
  END LOOP;
END $$;
