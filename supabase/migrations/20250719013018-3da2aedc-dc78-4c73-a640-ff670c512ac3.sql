-- Import Top 20 Notable Villages - Final Working Version
...
SELECT v.id, v.village_name || ' Development Association',
...
-- Insert development projects with correct project types
...
VALUES 
((SELECT id FROM villages WHERE village_name = 'Foumban'), 'Royal Palace Museum Renovation', 'other', 'Community-driven development project aimed at improving local infrastructure and economic opportunities.', 2021, 'ongoing', 'Government', 1500000000),
((SELECT id FROM villages WHERE village_name = 'Buea Town'), 'University Campus Expansion', 'school', 'University infrastructure development project', 2022, 'ongoing', 'Government', 2000000000),
((SELECT id FROM villages WHERE village_name = 'Kribi (Grand Batanga)'), 'Deep Water Port Development', 'other', 'Port infrastructure development for economic growth', 2020, 'ongoing', 'International NGO', 5000000000),
((SELECT id FROM villages WHERE village_name = 'Tiko'), 'Palm Oil Processing Plant', 'other', 'Agricultural processing facility development', 2021, 'completed', 'Community Funding', 800000000),
((SELECT id FROM villages WHERE village_name = 'Nso (Kumbo)'), 'Coffee Cooperative Center', 'market', 'Agricultural cooperative center for coffee farmers', 2022, 'ongoing', 'Government', 600000000),
((SELECT id FROM villages WHERE village_name = 'Bafut'), 'Bafut Community Center', 'community_hall', 'Traditional palace community development project', 2021, 'planned', 'Community Funding', 400000000),
((SELECT id FROM villages WHERE village_name = 'Ebolowa'), 'Ebolowa Health Center', 'health_center', 'Primary healthcare facility development', 2023, 'ongoing', 'International NGO', 700000000),
((SELECT id FROM villages WHERE village_name = 'Dschang'), 'Dschang Water Project', 'borehole', 'Clean water access improvement project', 2022, 'completed', 'Government', 300000000);
...
SELECT v.id, 'CamerPulse Data Import System', 'general',
  ...
  'approved', 'system@camerpulse.org'
FROM public.villages v 
WHERE v.verification_notes = 'Seeded entry - awaiting community verification';