-- Phase 6E: Integration & API Features
...
CREATE INDEX idx_user_integrations_service_id ON public.user_integrations(service_id);
...
CREATE INDEX idx_webhook_endpoints_url ON public.webhook_endpoints(endpoint_url);
...
CREATE INDEX idx_custom_connectors_user_id ON public.custom_connectors(user_id);
CREATE INDEX idx_custom_connectors_public ON public.custom_connectors(is_public) WHERE is_public = true;
CREATE INDEX idx_integration_analytics_integration_id ON public.integration_analytics(integration_id);
CREATE INDEX idx_integration_analytics_period ON public.integration_analytics(period_start, period_end);
