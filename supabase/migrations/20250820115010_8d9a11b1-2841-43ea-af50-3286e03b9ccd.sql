-- Phase 6: Advanced Security & Permission System (Skip existing enum)
...
-- Assign permissions to roles (safe with ON CONFLICT)
...
-- Create RLS policies for security_audit_logs (safe with IF NOT EXISTS)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'security_audit_logs' AND policyname = 'Admins can view audit logs') THEN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'security_audit_logs' AND policyname = 'System can insert audit logs') THEN
...
BEGIN
    ...
    window_start := NOW() - (_window_hours || ' hours')::INTERVAL;
    ...
    INSERT INTO public.rate_limits (
        identifier_type, identifier_value, action_type, request_count
    ) VALUES (
        _identifier_type, _identifier_value, _action_type, 1
    )
    ON CONFLICT (identifier_type, identifier_value, action_type, window_start)
