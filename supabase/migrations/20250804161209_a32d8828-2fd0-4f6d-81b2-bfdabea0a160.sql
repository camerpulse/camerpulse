-- Phase 1: Create security audit logs table and fix critical RLS policies
...
CREATE POLICY "Users can manage their captcha verifications" ON public.poll_captcha_verifications
...
CREATE POLICY "Users can view their rate limits" ON public.poll_rate_limits
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Admins can manage all rate limits" ON public.poll_rate_limits
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.user_roles 
      WHERE user_id = auth.uid() AND role = 'admin'::app_role
...
-- Add missing RLS policies for debt_predictions
CREATE POLICY "Users can insert debt predictions" ON public.debt_predictions
  FOR INSERT WITH CHECK (true);

-- Add missing RLS policies for civic_shield_moderators  
CREATE POLICY "Public can view moderator info" ON public.civic_shield_moderators
