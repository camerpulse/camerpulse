-- Extended Senator System Enhancements (Fixed Unique Constraint)
...
-- Add constraints for claim status (check if not exists first)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'chk_senator_claim_status' 
    AND table_name = 'senators'
  ) THEN
    ALTER TABLE public.senators 
    ADD CONSTRAINT chk_senator_claim_status 
...
-- Enhance senator_ratings table with new columns
DO $$ 
BEGIN
  BEGIN
...
-- Create senator dashboard analytics table (Fixed)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'senator_analytics_unique_metric' 
    AND table_name = 'senator_analytics'
  ) THEN
    ALTER TABLE public.senator_analytics 
    ADD CONSTRAINT senator_analytics_unique_metric UNIQUE(senator_id, metric_type, metric_date);
  END IF;
END $$;
