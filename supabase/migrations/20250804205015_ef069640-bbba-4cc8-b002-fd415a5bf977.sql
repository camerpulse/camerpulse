-- Phase 1: Disable Ashen AI System and Archive Complex Features (Fixed)
...
SET 
  ...
  error_logs = jsonb_set(
    ...
    ARRAY[array_length(array(select jsonb_array_elements(COALESCE(error_logs, '[]'::jsonb))), 1)]::text[],
    jsonb_build_object(
      'message', 'Plugin system frozen during Phase 1 simplification',
      'timestamp', now()::text
    )
  )
...
SET 
  ...
  edit_history = jsonb_set(
    ...
    ARRAY[array_length(array(select jsonb_array_elements(COALESCE(edit_history, '[]'::jsonb))), 1)]::text[],
    jsonb_build_object(
