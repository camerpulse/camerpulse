-- Recreate backup schema and logging tables, then run corrected cleanup
    ...
    IF v_text_cols IS NOT NULL AND array_length(v_text_cols,1) > 0 THEN
      v_sql := format(
        'SELECT array_agg(%1$I), jsonb_agg(jsonb_build_object(''sample_col'', %2$I)) FROM (
           SELECT %1$I, %2$I FROM public.%3$I WHERE (%4$s) AND (%5$s) LIMIT 5
         ) s',
    ...
    ELSE
      v_sql := format(
        'SELECT array_agg(%1$I), jsonb_build_array() FROM (
           SELECT %1$I FROM public.%2$I WHERE (%3$s) AND (%4$s) LIMIT 5
         ) s',
