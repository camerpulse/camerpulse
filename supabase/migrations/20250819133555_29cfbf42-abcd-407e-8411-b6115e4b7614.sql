-- Fix missing RLS policies for critical tables (using correct column names)

-- API Integrations - Admin only access (no user_id column, so admin-only)
...
-- Company Metrics - Admins only
DROP POLICY IF EXISTS "Admin access for company_metrics" ON company_metrics;
CREATE POLICY "Admin access for company_metrics" 
...
-- Insurance Claims - Users can manage their own claims via created_by
...
-- Insurance Policies - Admin only access (no user column)
DROP POLICY IF EXISTS "Admin access for insurance_policies" ON insurance_policies;
CREATE POLICY "Admin access for insurance_policies" 
