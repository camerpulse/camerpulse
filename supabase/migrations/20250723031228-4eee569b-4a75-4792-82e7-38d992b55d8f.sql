-- Create comprehensive bidding system tables
CREATE TABLE IF NOT EXISTS public.tender_bids (
  ...
  bid_status TEXT NOT NULL DEFAULT 'submitted' CHECK (bid_status IN ('draft', 'submitted', 'under_review', 'shortlisted', 'rejected', 'accepted', 'withdrawn')),
...
CREATE TABLE IF NOT EXISTS public.bid_evaluation_criteria (
  ...
  criteria_type TEXT NOT NULL DEFAULT 'technical' CHECK (criteria_type IN ('technical', 'financial', 'experience', 'compliance')),
...
CREATE TRIGGER update_bid_evaluations_updated_at
BEFORE UPDATE ON public.bid_evaluations
FOR EACH ROW
EXECUTE FUNCTION public.update_tender_updated_at();
