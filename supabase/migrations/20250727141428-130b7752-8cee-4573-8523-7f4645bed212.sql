...
-- Create village photo gallery entries using allowed photo types
INSERT INTO village_photos (village_id, photo_url, caption, photo_type)
SELECT 
  v.id,
  ...
  v.village_name || ' overview',
  'main'
FROM villages v
WHERE v.id IN (SELECT id FROM villages ORDER BY created_at DESC LIMIT 15);
