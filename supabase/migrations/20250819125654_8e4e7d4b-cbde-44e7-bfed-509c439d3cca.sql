-- Security migration: add safe helper and idempotent RLS policies for critical tables

-- 0) Helper: admin checker using existing has_role
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='shipments' AND policyname='Users can view shipments they are involved with'
  ) THEN
    CREATE POLICY "Users can view shipments they are involved with" ON public.shipments
    FOR SELECT USING (
      sender_id = auth.uid() OR recipient_id = auth.uid() OR public.is_admin(auth.uid())
    );
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='shipments' AND policyname='Users can create shipments'
  ) THEN
