-- Enhanced diaspora security and audit tables

-- Transaction logs table for all diaspora financial activities
CREATE TABLE public.diaspora_transaction_logs (
  ...
  transaction_type TEXT NOT NULL, -- 'donation', 'investment', 'remittance'
  amount_fcfa BIGINT NOT NULL,
  amount_original_currency NUMERIC,
  original_currency TEXT DEFAULT 'FCFA',
  exchange_rate NUMERIC DEFAULT 1.0,
  project_id UUID REFERENCES public.diaspora_investment_projects(id),
  payment_method TEXT, -- 'flutterwave', 'mobile_money', 'bank_transfer'
  payment_reference TEXT NOT NULL,
  payment_status TEXT NOT NULL DEFAULT 'pending',
  qr_receipt_data JSONB,
...
-- Virtual town halls and events system
CREATE TABLE public.diaspora_virtual_townhalls (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  title TEXT NOT NULL,
  description TEXT,
...
CREATE TABLE public.diaspora_townhall_registrations (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Diaspora civic engagement tracking
CREATE TABLE public.diaspora_civic_engagement (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  diaspora_profile_id UUID REFERENCES public.diaspora_profiles(id) ON DELETE CASCADE,
...
-- QR receipt system
CREATE TABLE public.diaspora_qr_receipts (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Cross-platform integration tracking
CREATE TABLE public.diaspora_cross_platform_actions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  diaspora_profile_id UUID REFERENCES public.diaspora_profiles(id) ON DELETE CASCADE,
...
ON public.diaspora_transaction_logs
FOR SELECT
USING (EXISTS (
  SELECT 1 FROM public.diaspora_profiles
  WHERE diaspora_profiles.id = diaspora_transaction_logs.diaspora_profile_id
...
CREATE POLICY "Admins can manage all townhalls"
ON public.diaspora_virtual_townhalls
FOR ALL
USING (EXISTS (
...
-- Insert default Flutterwave configuration
...
CREATE TRIGGER update_diaspora_transaction_logs_updated_at
  BEFORE UPDATE ON public.diaspora_transaction_logs
  FOR EACH ROW
  EXECUTE FUNCTION public.update_diaspora_updated_at();

CREATE TRIGGER update_diaspora_virtual_townhalls_updated_at
  BEFORE UPDATE ON public.diaspora_virtual_townhalls
  FOR EACH ROW
  EXECUTE FUNCTION public.update_diaspora_updated_at();
