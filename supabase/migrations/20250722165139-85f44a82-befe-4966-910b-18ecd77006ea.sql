...
-- Create storage policies for audit documents
...
CREATE POLICY "Approved audits are publicly viewable" 
ON storage.objects 
FOR SELECT 
USING (
  bucket_id = 'audit-documents' AND (
    -- Allow access if audit is approved
    EXISTS (
...
USING (
  ...
  EXISTS (
    ...
    AND ar.document_status IN ('pending_review', 'needs_revision')
  )
);

CREATE POLICY "Admins can delete audit documents" 
ON storage.objects 
FOR DELETE 
USING (
  bucket_id = 'audit-documents' AND
