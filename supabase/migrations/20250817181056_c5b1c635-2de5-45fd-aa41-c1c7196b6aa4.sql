...
CREATE POLICY IF NOT EXISTS "Pharmacies are viewable by everyone" ON public.pharmacies
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Authenticated users can add pharmacies" ON public.pharmacies
  FOR INSERT WITH CHECK (auth.uid() = submitted_by);

CREATE POLICY IF NOT EXISTS "Pharmacy owners can update their pharmacies" ON public.pharmacies
  FOR UPDATE USING ((auth.uid() = claimed_by) OR (auth.uid() = submitted_by));

CREATE POLICY IF NOT EXISTS "Admins can manage all pharmacies" ON public.pharmacies
  FOR ALL USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Companies are viewable by everyone" ON public.companies
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Authenticated users can add companies" ON public.companies
  FOR INSERT WITH CHECK (auth.uid() = submitted_by);

CREATE POLICY IF NOT EXISTS "Company owners can update their companies" ON public.companies
  FOR UPDATE USING ((auth.uid() = claimed_by) OR (auth.uid() = submitted_by));

CREATE POLICY IF NOT EXISTS "Admins can manage all companies" ON public.companies
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM user_roles 
      WHERE user_id = auth.uid() AND role = 'admin'
...
CREATE TRIGGER IF NOT EXISTS trigger_update_pharmacy_updated_at
  BEFORE UPDATE ON public.pharmacies
  FOR EACH ROW
  EXECUTE FUNCTION public.update_pharmacy_updated_at();

CREATE TRIGGER IF NOT EXISTS trigger_update_company_updated_at
