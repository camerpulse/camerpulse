...
-- Function to generate politician slug
CREATE OR REPLACE FUNCTION public.generate_politician_slug(politician_name TEXT, role_name TEXT DEFAULT '')
RETURNS TEXT AS $$
DECLARE
  base_slug TEXT;
  final_slug TEXT;
...
BEGIN
  ...
  base_slug := lower(trim(regexp_replace(
    CASE WHEN role_name != '' THEN role_name || '-' || politician_name ELSE politician_name END,
    '[^a-zA-Z0-9\s]', '', 'g'
  )));
