...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'admin_dashboard_modules_module_name_key'
  ) THEN
    ALTER TABLE admin_dashboard_modules ADD CONSTRAINT admin_dashboard_modules_module_name_key UNIQUE (module_name);
  END IF;
END $$;
...
-- 3. Handle existing notification_templates table
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notification_templates') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notification_templates' AND column_name = 'template_type') THEN
      ALTER TABLE notification_templates ADD COLUMN template_type text DEFAULT 'email';
      ALTER TABLE notification_templates ADD CONSTRAINT notification_templates_template_type_check CHECK (template_type IN ('email', 'push', 'in_app'));
    END IF;
...
-- 4. Optimize sentiment analysis - keep only core functionality
...
-- 6. Create triggers for simplified tables
