-- Add traditional leader fields to villages table first
ALTER TABLE villages 
ADD COLUMN IF NOT EXISTS traditional_leader_name TEXT,
ADD COLUMN IF NOT EXISTS traditional_leader_title TEXT DEFAULT 'chief';
  ...
  IF NEW.traditional_leader_name IS NOT NULL AND TRIM(NEW.traditional_leader_name) != '' THEN
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO traditional_leaders (
        ...
        COALESCE(NEW.traditional_leader_title, 'chief'),
        NEW.id,
        NEW.region,
        NEW.division,
        NEW.subdivision,
...
-- Create triggers
DROP TRIGGER IF EXISTS trigger_create_traditional_leader_from_village ON villages;
...
DROP TRIGGER IF EXISTS trigger_update_traditional_leader_from_village ON villages;
