-- Strengthen cleanup: avoid FK violations by only deleting rows not referenced by children
  ...
  LOOP
    ...
    LOOP
      ...
      v_not_refd := v_not_refd || format(' AND NOT EXISTS (SELECT 1 FROM public.%I c WHERE c.%I = %I.%I)', 
                        r_ref.child_table, r_ref.child_column, r_table.table_name, v_pk_name);
    END LOOP;
    ...
    -- Backup rows
    v_sql := format('INSERT INTO cleanup_backup.%I SELECT * FROM public.%I WHERE %s', r_table.table_name, r_table.table_name, v_where);
