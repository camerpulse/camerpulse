-- Phase 3: Tender Management & Bidding System Database Schema
...
-- Create tender bids table (from existing schema, but ensure it exists)
CREATE TABLE IF NOT EXISTS public.tender_bids (
  ...
  -- Bid details
  bid_amount_fcfa BIGINT NOT NULL,
  bid_amount_usd BIGINT,
  currency TEXT NOT NULL DEFAULT 'XAF',
  bid_validity_days INTEGER NOT NULL DEFAULT 30,
...
CREATE POLICY "Admins can manage all documents" ON public.tender_documents
FOR ALL USING (
  EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'admin'::app_role)
);

-- RLS Policies for tender_bids
CREATE POLICY "Bidders can view their own bids" ON public.tender_bids
FOR ALL USING (bidder_id = auth.uid());

CREATE POLICY "Tender creators can view bids for their tenders" ON public.tender_bids
FOR SELECT USING (
  EXISTS (SELECT 1 FROM tenders WHERE id = tender_bids.tender_id AND created_by = auth.uid())
);

CREATE POLICY "Admins can view all bids" ON public.tender_bids
FOR ALL USING (
  EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'admin'::app_role)
);
...
CREATE INDEX IF NOT EXISTS idx_tender_bids_bidder_id ON public.tender_bids(bidder_id);
CREATE INDEX IF NOT EXISTS idx_tender_bids_status ON public.tender_bids(status);
