...
-- Final corrected version with proper leader types
...
-- Insert village leaders with proper leader_type values
INSERT INTO public.village_leaders (village_id, leader_type, leader_name, start_year, is_current, bio, achievements)
SELECT 
  v.id,
  CASE 
    WHEN v.village_name IN ('Foumban', 'Ngaoundere (Lamido Palace Area)') THEN 'don'
    ELSE 'chief'
  END,
  CASE v.village_name
...
-- Insert development projects for select villages
...
-- Insert notable billionaires/diaspora elites for major villages
...
-- Insert celebrities for major villages
...
-- Update villages with realistic development scores
...
-- Add system contributions documenting the seeded data
