-- Fix data types and extend existing polls system with Pulse Polls Pro features
...
CREATE OR REPLACE FUNCTION public.get_poll_analytics_summary(p_poll_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  FROM public.poll_votes
  WHERE poll_id = p_poll_id::text AND user_id IS NOT NULL;
