-- Performance Optimization: Vote counting triggers and database optimizations
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        -- Update option counts in poll_options table if exists
        INSERT INTO poll_options (poll_id, option_text, vote_count, option_order)
        SELECT NEW.poll_id, 
...
CREATE INDEX IF NOT EXISTS idx_poll_votes_session_id_poll_id ON poll_votes (session_id, poll_id);
CREATE INDEX IF NOT EXISTS idx_poll_votes_region_created_at ON poll_votes (region, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_poll_votes_ip_hash_created_at ON poll_votes (hashed_ip, created_at DESC);

-- Composite indexes for fraud detection
CREATE INDEX IF NOT EXISTS idx_poll_votes_fraud_detection ON poll_votes (poll_id, hashed_ip, created_at);
...
CREATE INDEX IF NOT EXISTS idx_polls_category_active ON polls (poll_type, is_active);
...
-- Schedule automatic statistics refresh (every 5 minutes)
SELECT cron.schedule(
    'refresh-poll-statistics',
    '*/5 * * * *',
    'SELECT refresh_poll_statistics();'
);
...
SELECT cron.schedule(
    ...
    '0 2 * * *',
    $$
    DELETE FROM poll_rate_limits 
    WHERE window_start < NOW() - INTERVAL '7 days';
    
    DELETE FROM poll_captcha_verifications 
