-- Phase 3: Advanced Analytics & Intelligence Migration
...
-- Sentiment trends and aggregations
CREATE TABLE public.sentiment_trends (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    topic TEXT NOT NULL,
    entity_type TEXT, -- 'politician', 'party', 'policy', 'region'
    entity_id UUID,
    time_period DATE NOT NULL,
    aggregation_level TEXT NOT NULL DEFAULT 'daily', -- 'hourly', 'daily', 'weekly', 'monthly'
    total_mentions INTEGER NOT NULL DEFAULT 0,
    positive_count INTEGER NOT NULL DEFAULT 0,
    negative_count INTEGER NOT NULL DEFAULT 0,
    neutral_count INTEGER NOT NULL DEFAULT 0,
    average_sentiment NUMERIC(5,2) NOT NULL DEFAULT 0.0,
    sentiment_change_from_previous NUMERIC(5,2),
    trending_keywords TEXT[],
    geographic_breakdown JSONB DEFAULT '{}',
    demographic_breakdown JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE(topic, entity_type, entity_id, time_period, aggregation_level)
);
...
CREATE POLICY "Public can view sentiment analysis" ON public.sentiment_analysis
FOR SELECT USING (true);

CREATE POLICY "Public can view sentiment trends" ON public.sentiment_trends
FOR SELECT USING (true);
...
CREATE INDEX idx_sentiment_analysis_source_created ON public.sentiment_analysis(content_source, created_at);
CREATE INDEX idx_sentiment_analysis_entities ON public.sentiment_analysis USING GIN(entities_mentioned);
CREATE INDEX idx_sentiment_trends_topic_period ON public.sentiment_trends(topic, time_period, aggregation_level);
CREATE INDEX idx_sentiment_trends_entity ON public.sentiment_trends(entity_type, entity_id, time_period);
...
-- Sample sentiment trends
INSERT INTO public.sentiment_trends (
  topic, entity_type, time_period, aggregation_level, total_mentions, 
  positive_count, negative_count, neutral_count, average_sentiment, trending_keywords
) VALUES
  (
    'Healthcare Reform',
    'policy',
    ...
    89,
    0.15,
    ARRAY['universal healthcare', 'medical access', 'rural hospitals', 'insurance coverage']
  ),
  (
    'Education Budget',
    'policy',
    ...
    53,
    0.32,
    ARRAY['school funding', 'teacher salaries', 'infrastructure', 'digital learning']
  );

-- Sample custom reports
INSERT INTO public.custom_reports (
  user_id, report_name, report_type, report_description, data_sources, 
  filter_criteria, is_scheduled, recipients
) VALUES
  (
    '00000000-0000-0000-0000-000000000000', -- placeholder user_id
    'Weekly Civic Engagement Summary',
    'weekly_analysis',
    'Comprehensive analysis of civic engagement activities, including polling participation, educational content consumption, and sentiment trends.',
    '["poll_data", "civic_education_data", "sentiment_data"]'::jsonb,
    '{"time_range": "7_days", "regions": ["all"], "content_types": ["all"]}'::jsonb,
    true,
    '{}'::UUID[]
  );
...
DECLARE
  ...
  poll_participation_rate NUMERIC;
  education_completion_rate NUMERIC;
  result JSONB;
BEGIN
...
BEGIN
  -- Sample prediction generation logic
  ...
  SELECT 
    (SELECT id FROM public.predictive_models WHERE model_type = 'civic_participation' AND is_active = true LIMIT 1),
    'civic_participation',
    'region',
...
COMMENT ON TABLE public.sentiment_analysis IS 'Sentiment analysis results from various sources';
COMMENT ON TABLE public.sentiment_trends IS 'Aggregated sentiment trends over time';
