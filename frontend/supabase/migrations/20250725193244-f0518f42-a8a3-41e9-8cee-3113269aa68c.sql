...
CREATE OR REPLACE FUNCTION public.check_claim_renewals()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  UPDATE public.artist_profile_claims 
  SET status = 'expired'
...
INSERT INTO public.poll_bot_patterns (pattern_type, pattern_data, risk_score, is_active) VALUES
('user_agent', '{"patterns": ["bot", "spider", "crawler", "scraper", "automation", "headless", "phantom", "selenium"]}', 90, true),
('behavioral', '{"patterns": ["rapid_voting", "sequential_ips", "identical_fingerprints"]}', 80, true),
('device', '{"patterns": ["missing_fingerprint", "generic_fingerprint", "suspicious_screen_resolution"]}', 70, true)
