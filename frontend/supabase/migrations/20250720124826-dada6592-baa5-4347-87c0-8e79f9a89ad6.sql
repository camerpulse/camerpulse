-- Phase 4B: Advanced Analytics & AI-Powered Features
...
CREATE TABLE public.analytics_metrics (
  ...
  aggregation_period TEXT NOT NULL DEFAULT 'hour', -- 'minute', 'hour', 'day', 'week', 'month'
...
CREATE TABLE public.notification_channels (
  ...
  channel_type TEXT NOT NULL, -- 'email', 'sms', 'push', 'webhook', 'slack', 'teams'
...
CREATE TABLE public.notification_templates (
  ...
  channel_types TEXT[] NOT NULL DEFAULT ARRAY['email'],
  is_active BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
CREATE TABLE public.optimization_recommendations (
  ...
  implementation_effort TEXT NOT NULL DEFAULT 'medium', -- 'low', 'medium', 'high'
...
INSERT INTO public.notification_channels (channel_name, channel_type, channel_config) VALUES
('Email Primary', 'email', '{"smtp_host": "smtp.example.com", "port": 587}'),
('SMS Gateway', 'sms', '{"provider": "twilio", "region": "us"}'),
('Slack Alerts', 'slack', '{"webhook_url": "", "channel": "#alerts"}'),
('Push Notifications', 'push', '{"provider": "firebase", "project_id": ""}');

INSERT INTO public.notification_templates (template_name, template_type, subject_template, body_template, channel_types) VALUES
('Civic Alert', 'alert', 'Civic Alert: {{alert_type}}', 'A new civic issue has been reported: {{description}}', ARRAY['email', 'push']),
('Performance Alert', 'performance', 'Performance Alert: {{metric_name}}', 'Performance metric {{metric_name}} has exceeded threshold: {{current_value}}', ARRAY['email', 'slack']),
('Weekly Report', 'report', 'Weekly Analytics Report', 'Your weekly analytics summary is ready. {{summary}}', ARRAY['email']);
