-- Create tables for AI Civic Strategist Core
...
-- Table for storing reusable strategy patterns
CREATE TABLE public.strategy_patterns (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  pattern_name TEXT NOT NULL,
  pattern_type TEXT NOT NULL,
  problem_categories TEXT[] DEFAULT ARRAY[]::TEXT[],
  solution_template JSONB NOT NULL,
  success_rate NUMERIC DEFAULT 0.0,
  usage_count INTEGER DEFAULT 0,
  regions_effective TEXT[] DEFAULT ARRAY[]::TEXT[],
...
CREATE INDEX idx_strategy_patterns_type ON public.strategy_patterns(pattern_type);
...
-- Function to analyze strategy problem
...
BEGIN
  ...
  FOR related_patterns IN
    ...
    WHERE p_category = ANY(problem_categories)
    AND is_active = true
  ...
  solution_blueprint := solution_blueprint || jsonb_build_object(
    ...
    'recommended_timeline', CASE 
      WHEN complexity_score <= 3 THEN '3-6 months'
      WHEN complexity_score <= 6 THEN '6-12 months'
      ELSE '12-24 months'
    END,
