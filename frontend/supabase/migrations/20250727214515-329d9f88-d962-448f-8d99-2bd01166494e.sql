-- Create enhanced chat system with full functionality
...
-- Create default support bot conversation for demo
DO $$
DECLARE
  v_support_bot_id UUID;
  v_conversation_id UUID;
BEGIN
  -- Create or get support bot user
  INSERT INTO public.chat_bots (
    ...
    'AI-powered customer support assistant with voice and file capabilities',
    'You are a helpful customer support assistant. Provide clear, concise, and friendly responses to user questions. You can help with file sharing, voice messages, and general support inquiries.',
    true,
    'support',
    'support-bot'
  ) 
  ON CONFLICT (name) DO UPDATE SET
    ...
    updated_at = now()
  RETURNING id INTO v_support_bot_id;

  -- Create demo conversation for current user if they're authenticated
