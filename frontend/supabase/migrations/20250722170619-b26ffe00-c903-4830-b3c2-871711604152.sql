-- Create investigation requests table (fixed column reference)
...
-- Function to calculate corruption trends (fixed column reference)
...
BEGIN
  ...
  WITH regional_stats AS (
    ...
    WHERE ar.created_at >= start_date
      ...
      AND ar.status = 'approved'  -- Fixed column name
    GROUP BY ar.region
  ),
  ...
  SELECT 
    ...
    cm.corruption_index,
    NULL::DECIMAL as prev_value,  -- Simplified for now
